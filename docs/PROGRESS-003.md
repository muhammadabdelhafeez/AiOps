# KFH AIOps Platform â€” Progress Log Volume 003
...
- **Phase:** <1â€“7 per Development Strategy in copilot-instructions.md>
...
- **Summary:** <1â€“3 sentences describing what was done and why>
- **Files touched:**
  - `path/to/file1`
  - `path/to/file2`
...
- **Definition of Done checklist (from copilot-instructions Â§25):**
...
| Area | Status | Notes |
|------|--------|-------|
| Phase 1 â€” Modular monolith skeleton | ðŸŸ¡ In progress | Package structure established under `org.kfh.aiops`; frontend-aligned `/api/v1/**` scaffold endpoints added |
| Phase 2 â€” Custom Log Index Engine | âšª Not started | |
| Phase 3 â€” Neo4j banking flow graph | âšª Not started | |
| Phase 4 â€” RCA evidence builder + causal scoring | âšª Not started | |
| Phase 5 â€” AI Router (DeepSeek + Azure OpenAI) | ðŸŸ¡ In progress | Settings-managed Azure OpenAI provider readiness now includes GPT 5.4 / Responses API / Entra ID configuration |
| Phase 6 â€” Redis hot health state + dashboard cache | âšª Not started | |
| Phase 7 â€” Worker extraction | âšª Not started | |

Legend: ðŸŸ¢ Done  ðŸŸ¡ In progress  ðŸ”´ Blocked  âšª Not started
...
### 2026-07-01 â€” Index storage provider SPI (LOCAL/NFS/SMB/PVC/S3/Azure) for OS/OpenShift portability
- **Phase:** 2
- **Module(s):** index (`org.kfh.aiops.index`), platform.config (SettingsService), frontend/settings
- **Type:** feature
- **Country/Tenant scope:** ALL
- **Summary:** Made index storage **pluggable by type** so the app works unchanged across Linux, Windows, and OpenShift. New `IndexStorageType` (LOCAL/NFS/SMB/PVC/S3/AZURE_BLOB, `isFilesystem()`), `IndexStorageProvider` SPI + `IndexStorageProviderRegistry`. `FilesystemIndexStorageProvider` covers LOCAL (Linux/Windows disk), NFS, SMB (Windows/UNC), and PVC (OpenShift) â€” all resolve to a `java.nio.Path`, so the engine is OS-agnostic. `S3IndexStorageProvider`/`AzureBlobIndexStorageProvider` are registered stubs (throw until the object-storage SDK is added). `SettingsService.resolveIndexStorage` now returns `{provider, endpoint, bucket, region}`; `IndexStorageResolver` selects the provider by type and falls back to `kfh.index.storage.path` for not-yet-wired object storage. UI: Index Storage **Provider** dropdown offers LOCAL/NFS/SMB/PVC/S3/AZURE_BLOB with **provider-aware form fields** (filesystem â†’ single Path field; S3/Azure â†’ URI + bucket/region/keys) that re-render on provider change. Connector tester (`DefaultInfrastructureConnectionTester.testIndexStorage`) now validates **LOCAL/NFS/SMB/PVC as real filesystem directories** (absolute + exists + readable/writable â€” Test/Test&Save do real checks); S3/Azure remain URI/metadata validation only until the object-storage SDK is added.
- **Files touched:**
  - `src/main/java/org/kfh/aiops/index/` (IndexStorageType, IndexStorageProvider, FilesystemIndexStorageProvider, S3IndexStorageProvider, AzureBlobIndexStorageProvider, IndexStorageProviderRegistry â€” new; IndexStorageResolver rewired)
  - `src/main/java/org/kfh/aiops/platform/config/SettingsService.java` (`resolveIndexStorage` â†’ provider+endpoint)
  - `src/main/resources/static/pages/settings/settings.js` (provider options + help)
- **DB migrations / API changes:** N/A
- **Tests added/updated:** `IndexStorageTypeTest`, `IndexStorageProviderRegistryTest`, `FilesystemIndexStorageProviderTest`, `S3IndexStorageProviderTest` (new) + `IndexStorageResolverTest`/writer/search (updated) â€” 40 index tests green; `mvn test` clean
- **Docs updated:** docs/SERVICES_CORE.md, docs/PROGRESS-003.md
- **Follow-ups / TODO:** implement S3/Azure providers (add SDK deps + object-storage segment model, since object stores have no append); the `endpoint` path must be reachable by the app process (Windows Tomcat â†’ local disk or UNC share; OpenShift â†’ PVC mount).
- **Author:** claude-code
- **Correlation:** Phase 2 storage SPI

### 2026-07-01 â€” Log Explorer (Kibana-Discover) UI over /logs/search
- **Phase:** 2 (frontend)
...
- **Summary:** New **Log Explorer** page â€” a Kibana-Discover-style telemetry search over the Custom Index Engine. Self-mounting vanilla page (no React/Babel) rendering into `#page-root`: filter bar (free-text, time range, kind, severity, source, service, trace id), results table (time/severity/source/service/resource/message), result count + `tookMs`, and prev/next pagination. All server data HTML-escaped. Wired via `APIClient.logs.search(query)` â†’ `POST /api/v1/logs/search`; added a `PAGES.explorer` router entry and an Operations nav item.
- **Files touched:**
...
- **Tests added/updated:** N/A (static SPA â€” requires browser smoke test; not verifiable in the JDK/Node-less build env)
...
### 2026-07-01 â€” Phase 2: Custom Index Engine â€” cold-shard archive + country-isolation guard (increment 3)
- **Phase:** 2
- **Module(s):** index (`org.kfh.aiops.index`)
- **Type:** feature + security
- **Country/Tenant scope:** ALL
- **Summary:** (Archive) `ArchiveStore` interface + `FilesystemArchiveStore` (gzip a cold shard's segment to `{archive.path}/{country}/{env}/{kind}/{date}/shard-NN/segment.jsonl.gz`); `IndexRetentionService` now **archives before delete** when `kfh.index.archive.enabled=true`. Cloud (S3/Azure Blob) archive is a drop-in `ArchiveStore` impl once SDK deps are added. (Security) Closed a country-isolation gap: `IndexSearchService` now calls `CountryAccessGuard.requireAccess(ctx, country)` before opening any shard, so a caller can only search their scoped country (cross/all-country needs `COUNTRY_GLOBAL_VIEW`) â€” complementing the top-level physical country partition.
- **Files touched:**
  - `src/main/java/org/kfh/aiops/index/` (ArchiveStore, FilesystemArchiveStore â€” new; SegmentStore `SEGMENT_FILE` public; IndexProperties `Archive`; IndexRetentionService archive-before-delete; IndexSearchService `CountryAccessGuard`)
...
- **Tests added/updated:** `FilesystemArchiveStoreTest` (new), `IndexRetentionServiceTest` (+archive-before-delete), `IndexSearchServiceTest` (+cross-country denial) â€” 28 index tests green; `mvn test` clean
- **Docs updated:** docs/SERVICES_CORE.md, docs/CAUSAL_PIPELINE.md Â§9, docs/SECURITY.md, docs/PROGRESS-003.md, .github/PROGRESS.md
- **Security / OWASP checklist:**
  - [x] A01 Broken Access Control â€” country isolation enforced at service layer before any shard read
...
### 2026-07-01 â€” Phase 2: Custom Index Engine â€” inverted index + retention + Settings-driven path (increment 2)
- **Phase:** 2
- **Module(s):** index (`org.kfh.aiops.index`), platform.config (SettingsService)
- **Type:** feature
- **Country/Tenant scope:** ALL
- **Summary:** Increment 2 of the Custom Index Engine. (A) In-shard inverted index: `ShardIndex` (parsed docs + fieldâ†’valueâ†’doc-index postings) + `ShardIndexCache` (per-shard, invalidated by segment byte-size) so repeat searches skip JSON re-parsing and resolve exact-match filters by posting-list intersection; `IndexSearchService` now scans via the cache. (B) `IndexRetentionService` purges expired shard date-dirs per-kind retention (`@Scheduled`; core method unit-tested). (C) `IndexStorageResolver` resolves the index root from the Settings INDEX_STORAGE connector (`SettingsService.resolveIndexStorage`, Part-D pattern) with `kfh.index.storage.path` fallback; `IndexWriterService.index` now takes `TenantContext` and both writer + searcher use the resolved root.
- **Files touched:**
  - `src/main/java/org/kfh/aiops/index/` (ShardIndex, ShardIndexCache, IndexRetentionService, IndexStorageResolver â€” new; SegmentStore `segmentSize`, IndexWriterService `+ctx`, IndexSearchService cache-backed)
  - `src/main/java/org/kfh/aiops/platform/config/SettingsService.java` (`resolveIndexStorage`)
- **DB migrations:** N/A
- **API changes:** N/A (search endpoint unchanged; writer signature added `ctx` â€” no external callers yet)
- **Tests added/updated:** `ShardIndexTest`, `ShardIndexCacheTest`, `IndexRetentionServiceTest`, `IndexStorageResolverTest` (new) + `IndexWriterServiceTest`/`IndexSearchServiceTest` (updated) â€” 24 index tests green; `mvn test` clean
- **Docs updated:** docs/SERVICES_CORE.md, docs/CAUSAL_PIPELINE.md Â§9, docs/PROGRESS-003.md, .github/PROGRESS.md
- **Security / OWASP checklist:**
...
### 2026-07-01 â€” Phase 2: Custom Index Engine â€” searchable core (increment 1)
- **Phase:** 2
...
- **Summary:** First increment of the Custom Log Index Engine (the Elasticsearch replacement, Â§10 / funnel Stage 3). Typed model (`TelemetryDocument`/`TelemetryKind`/`IndexQuery`/`IndexSearchResult`), sharded append-only `SegmentStore` (JSONL per `{country}/{env}/{kind}/{date}/shard-NN`), `IndexWriterService` (batched, hash-routed writes), and `IndexSearchService` (time-partition prune â†’ country/env filter â†’ parallel filtered scan, newest-first, paginated). Exposed `POST /api/v1/logs/search` (RBAC `ALERT_READ`, tenant/country scoped). `IndexProperties` binds `kfh.index.*` (shards-per-day, write-batch-size, search-parallelism, retention-days per kind).
- **Files touched:**
...
- **Tests added/updated:** `ShardKeyTest`, `SegmentStoreTest`, `IndexWriterServiceTest`, `IndexSearchServiceTest` (14 cases; temp-dir, no external deps) â€” `mvn test` green
- **Docs updated:** docs/SERVICES_CORE.md, docs/CAUSAL_PIPELINE.md Â§9, docs/PROGRESS-003.md, .github/PROGRESS.md
- **Security / OWASP checklist:**
...
### 2026-07-01 â€” Fix flaky Kafka Test Connection (5s timeout too tight; disable metrics push)
- **Phase:** 1 (Settings infrastructure testers)
- **Module(s):** platform.config (DefaultInfrastructureConnectionTester)
- **Type:** fix
- **Country/Tenant scope:** ALL
- **Summary:** The Settings Kafka Test authenticated fine (`Successfully logged in`) but `AdminClient.describeCluster()` (fetchMetadata) took ~3â€“5 s while the test timeout was the generic 5 s â€” so it passed once at 3248 ms then failed at 5109 ms with a bare `IllegalStateException`. Raised the Kafka test timeout floor to 15 s, set `enable.metrics.push=false` (KIP-714 telemetry handshake can keep a node from becoming "ready" against newer brokers â†’ fetchMetadata timeout), and added a clear operator message on metadata timeout (and a non-null fallback for other errors). Root aggravator: app ships kafka-clients 3.9.2 vs the 4.3.1 broker; aligning the broker to 3.9.x remains the alternative.
- **Files touched:**
...
### 2026-06-30 â€” Part D: runtime Redis client + fingerprint dedup (Phase 6 start)
...
- **Tests added/updated:** `RedisKeysTest`, `RedisConnectionSettingsTest`, `RedisSettingsResolverTest`, `FingerprintDedupServiceTest` (15 cases) â€” `mvn test` green, no Docker/Redis needed
- **Docs updated:** docs/SERVICES_SUPPORT.md, docs/CAUSAL_PIPELINE.md (Â§9), docs/RUNBOOKS.md, .github/PROGRESS.md
- **Security / OWASP checklist:**
  ...
  - [x] DB 0 only + key-prefix isolation (copilot-instructions Â§12)
...
- **Follow-ups / TODO:** Remaining Tier 0 item (de-duplicate copy-pasted JS helpers `esc`/`pageContent`/`toast`/`icons` into `shared/js` + Java `isSecretLike`/`normalize`/`firstNonBlank` into one util) requires a build/run to verify and should be done on a JDK-equipped machine. Tier 1: introduce typed domain model, split `SettingsService`/`ConnectorService`, break `platformâ†”commandcenter` package cycle.
- **Author:** claude-code
- **Correlation:** commit ba998b4

### 2026-06-30 â€” Docs realigned to Causal Funnel architecture (master design `docs/CAUSAL_PIPELINE.md`)

- **Phase:** Phase 1 (docs)
- **Module(s):** docs
- **Type:** docs
- **Country/Tenant scope:** ALL
- **Summary:** Introduced `docs/CAUSAL_PIPELINE.md` as the master architecture doc capturing the funnel that handles **100,000 alerts every 20 minutes with 1â€“3 Azure OpenAI 5.5 calls per cycle** by reducing raw events through ingestion â†’ normalization â†’ fingerprint+Redis dedup â†’ Custom Index Engine â†’ Neo4j blast radius â†’ business-journey impact filter â†’ deterministic causal scoring â†’ `EvidencePack` (â‰¤ 3 KB) â†’ AI summary cache â†’ AI router (DeepSeek first, Azure only for CRITICAL + customer impact + novel + executive). Codified the one-line principle "**code finds the root cause, AI explains it**", the AI router decision table, the EvidencePack contract (no secrets, no raw telemetry, stable evidence IDs), the indicative cost math (~$25â€“80/month per PROD env vs. ~$2.16M/month naive), the per-stage time budget (â‰¤ 8 min e2e), the seven cost-control levers, the six AI-never rules, the required `application.properties` throttles, and the degraded-mode behaviors. Realigned strategic docs (OVERVIEW, ARCHITECTURE, FLOWS, ROADMAP, OUTBOX, DATA_MODEL, BACKEND_MODULES, UI_PAGES) and pointer docs (README, SERVICES_CORE, SECURITY, API_CONTRACTS, RUNBOOKS, CODE_TEMPLATES, AI_CODING_ASSISTANT_KNOWLEDGE_GRAPH) to reference the master doc and the funnel-aligned event types, Redis key namespaces, and `EvidencePack` / `AiRouter` / `CostGuard` Java templates. Removed v1 wording (retrieval packs, SharePoint, GPT-5.2-Pro, two operating modes) in favor of the funnel framing. Added Â§22 *Causal Funnel Graph* to `AI_CODING_ASSISTANT_KNOWLEDGE_GRAPH.md` so coding agents load funnel rules first.
- **Files touched:**
...
- **DB migrations:** N/A
- **API changes:** N/A â€” but `API_CONTRACTS.md` now states explicitly that `/api/v1/rca/**` and `/api/v1/ai/**` accept only the `EvidencePack` / RCA-result contracts (no raw telemetry), and AI work is dispatched via outbox (`AI_NARRATIVE_REQUESTED`) returning `202 Accepted` + correlationId.
- **Tests added/updated:** N/A (docs-only). Future implementation tasks must add `EvidencePackContractTest` (size cap, no-PII regex, stable IDs), `AiRouterContractTest` (cache â†’ DeepSeek â†’ Azure decision matrix), `CostGuardTest`, and `IncidentLifecycleEngineTest` (deterministic transitions).
- **Docs updated:** all docs in `docs/` top level except `PROGRESS*.md`, `security-assessment-*.md`, `FRONTEND_MODULES.md`, `SERVICES_SUPPORT.md`, `DATABASE_SCHEMA.md`. Numbering of `AI_CODING_ASSISTANT_KNOWLEDGE_GRAPH.md` now reads Â§1 â†’ Â§22 in order.
- **Security / OWASP checklist:**
  - [x] Tenant + user context enforced â€” preserved; SECURITY.md now also requires `(tenant, country, environment)` scoping on every persistence + cache key.
  - [x] RBAC checked at service layer â€” preserved; SECURITY.md explicit on filters being defense-in-depth only.
  - [x] Inputs validated (Bean Validation) â€” preserved.
  - [x] Audit log written for write actions â€” preserved + extended: every AI router decision now audited (`model`, `reason`, `tokens`, `cost`, `confidence`, `correlationId`).
  - [x] No secrets / PII / tokens logged or returned â€” extended: `EvidencePack` forbids secrets/PII via `EvidencePackValidator` regex sweep; AI never receives raw telemetry; cited evidence IDs validated subset.
  - [x] SSRF-safe for any URL-based config â€” preserved (connector endpoint guard already in place).
- **Definition of Done checklist (from copilot-instructions Â§25):**
  ...
  - [x] Follows package/module boundaries (mapping in CAUSAL_PIPELINE Â§9 + BACKEND_MODULES.md)
  - [x] Audit logs for write actions
  - [x] No secrets exposed
  - [x] Tests for core logic â€” documented as required for the implementation phase (contract tests listed)
  - [x] Correlation ID supported
  - [x] Does not break incident lifecycle rules â€” explicitly reinforces deterministic lifecycle (Â§13)
  - [x] Does not bypass custom index engine for telemetry search â€” Elasticsearch/OpenSearch explicitly forbidden again
  - [x] Extractable into a future microservice â€” Phase 7 in ROADMAP.md
- **Follow-ups / TODO:**
  - Build Phase 2â€“5 modules per ROADMAP (ingestion, normalization, fingerprint, custom index, topology, RCA evidence/causal, AI router, CostGuard, deterministic lifecycle engine).
  - Add `application.properties` throttles listed in CAUSAL_PIPELINE Â§10 and AI_CODING_ASSISTANT_KNOWLEDGE_GRAPH Â§22.6.
...
- **DB migrations:** N/A
- **API changes:** N/A â€” `POST /api/v1/settings/{section}/test` response shape unchanged; only the `message` field is more descriptive on Redis failures.
- **Tests added/updated:** `DefaultInfrastructureConnectionTesterTest` â€” added `shouldDescribeProtectedModeRedisPingFailureWithServerReply`, `shouldDescribeNoAuthRedisPingFailureWithServerReply`, `shouldHintAtTlsOrProtectedModeWhenRedisRepliesAreEmptyOverPlainSocket`, `shouldHintAtCredentialsWhenRedisRepliesAreEmptyOverTls`, and updated `shouldReturnClearRedisAuthenticationGuidanceWhenPasswordIsWrong` to assert the new `Server reply:` suffix. 14 tests green.
- **Docs updated:** `docs/PROGRESS-003.md`
- **Security / OWASP checklist:**
  ...
  - [x] No secrets / PII / tokens logged or returned â€” Redis PING/AUTH server replies never contain the supplied password, and `sanitizeRedisReply` strips control chars and truncates to 200 chars
...
### 2026-06-29 â€” Settings infrastructure Redis test: stop returning HTTP 500 on draft payloads & add TLS support

- **Phase:** Phase 1
- **Module(s):** `platform.config`
- **Type:** fix, test
- **Country/Tenant scope:** ALL, KW, BH, EG
- **Summary:** Fixed the Settings â†’ Servers & Index â†’ Redis `Test Only` / `Test & Save` flow that started returning HTTP 500 (`Request could not be processed`) when the operator first opened the connector modal. Root cause: `InfrastructureTestConfig.from(...)` ran outside the tester's `try/catch` and used `Map.copyOf(...)`, which throws `NullPointerException` whenever the UI draft included a top-level `"lastTest": null` (the empty connector default). Payload parsing is now null-safe (uses a defensive `LinkedHashMap` that preserves null values without crashing) and is also wrapped in the tester's failure handler so any future payload-parsing problem surfaces as a structured `Fail` response instead of a 500. The same change also adds proper TLS socket support for Redis when the UI `TLS Enabled` toggle is on (previous code always opened a plain socket), with a clear error if the TLS handshake fails so operators are not misled into believing PING failed because of auth.
- **Files touched:**
  - `src/main/java/org/kfh/aiops/platform/config/DefaultInfrastructureConnectionTester.java`
  - `src/main/java/org/kfh/aiops/platform/config/InfrastructureTestConfig.java`
  - `src/test/java/org/kfh/aiops/platform/config/DefaultInfrastructureConnectionTesterTest.java`
- **DB migrations:** N/A
- **API changes:** N/A â€” `POST /api/v1/settings/{section}/test` response shape unchanged; the Redis `tlsEnabled` field already existed in the request schema and is now honored on the server side.
- **Tests added/updated:** `DefaultInfrastructureConnectionTesterTest` (`shouldReturnFailResultInsteadOfThrowingWhenPayloadContainsNullValues`, plus the existing Redis AUTH-helper tests). Full Settings suite (`SettingsServiceTest`, `DefaultInfrastructureConnectionTesterTest`) â€” 35 tests green.
- **Docs updated:** `docs/PROGRESS-003.md`
- **Security / OWASP checklist:**
  - [x] Tenant + user context enforced (unchanged â€” test endpoint still requires Settings context and returns correlation ID)
  - [x] RBAC checked at service layer (unchanged â€” `SETTINGS_WRITE` gate remains)
...
### 2026-06-29 â€” Settings infrastructure Redis test: support password-only AUTH fallback for `default` user

- **Phase:** Phase 1
- **Module(s):** `platform.config`
- **Type:** fix, test
- **Country/Tenant scope:** ALL, KW, BH, EG
- **Summary:** Corrected the Settings â†’ Servers & Index â†’ Redis `Test Only` / `Test & Save` backend probe so it no longer produces a misleading generic `Redis PING failed or requires authentication` result when the operator enters username `default` against Redis deployments that still expect password-only `AUTH <password>`. The Redis tester now first tries the supplied username/password pair, then safely retries password-only AUTH when the username is `default` and the server responds with an AUTH-shape error or wrong-password response typical of non-ACL/password-only setups. Failure messaging was also tightened so the UI now tells the operator to leave username blank unless the Redis server actually uses ACL users.
- **Files touched:**
  - `src/main/java/org/kfh/aiops/platform/config/DefaultInfrastructureConnectionTester.java`
  - `src/test/java/org/kfh/aiops/platform/config/DefaultInfrastructureConnectionTesterTest.java`
- **DB migrations:** N/A
- **API changes:** N/A â€” existing `POST /api/v1/settings/{section}/test` response shape is unchanged; only Redis test behavior and message quality improved.
- **Tests added/updated:** `DefaultInfrastructureConnectionTesterTest`
- **Docs updated:** `docs/PROGRESS-003.md`
- **Security / OWASP checklist:**
  - [x] Tenant + user context enforced (unchanged â€” test endpoint still requires Settings context and returns correlation ID)
  - [x] RBAC checked at service layer (unchanged â€” `SETTINGS_WRITE` gate remains in Settings service/controller flow)
  ...
  - [x] SSRF-safe for any URL-based config (unchanged â€” Redis host validation still blocks loopback/link-local/metadata targets)
- **Definition of Done checklist (from copilot-instructions Â§25):**
  - [x] Supports tenant + country context
  - [x] Clear DTOs and validation
...
- **Author:** copilot-agent
- **Correlation:** user request â€” "in the UI its show test failed correct it then"

### 2026-06-29 â€” Neo4j Topology Graph: country-level scope in Settings (KW / BH / EG / ALL)

- **Phase:** Phase 1
- **Module(s):** `platform.config` (backend `SettingsService`, `JdbcSettingsMetadataStore`) + `commandcenter.settings` (frontend SPA)
- **Type:** feature
- **Country/Tenant scope:** ALL, KW, BH, EG
- **Summary:** Brought the Settings â†’ Databases â†’ Edit Neo4j Topology Graph popup in line with the Azure OpenAI integrations and database/SharePoint/infrastructure connector rows: the operator now picks one or more country scopes (`ALL`, `KW`, `BH`, `EG`) on the Neo4j row, and the backend persists the row under that scope in `config.integration_settings`. `JdbcSettingsMetadataStore.countryScopesFor` now routes `neo4j` through the same `scopesFromItem(...)` helper used by `azureOpenAI.integrations[]` and `databases.connections[]`, so a row saved with `countryCodes:["ALL"]` is durable for any country session, while `countryCodes:["KW"]` is durable only for Kuwait. `SettingsService.sanitizeNeo4j` now threads `TenantContext` through and emits `countryCode` + `countryCodes` on the response, matching the documented response contract for all other list-backed sections. The frontend Edit Neo4j Topology Graph modal now renders a country multi-select (Hold Ctrl to pick 1 or 2 countries; All Countries applies to KW, BH, and EG) and includes the picked scope in both `POST /api/v1/settings/neo4j/test` and `PUT /api/v1/settings` requests.
- **Files touched:**
  - `src/main/java/org/kfh/aiops/platform/config/SettingsService.java` â€” `sanitizeNeo4j(ctx, ...)`, country fields on the boot snapshot from `neo4j()`, country pass-through in `neo4jTestRequest`
  - `src/main/java/org/kfh/aiops/platform/config/JdbcSettingsMetadataStore.java` â€” `countryScopesFor` routes `neo4j` through `scopesFromItem`
  - `src/main/resources/static/pages/settings/settings.js` â€” country chips in the Neo4j modal, `updateNeo4jCountries`, normalization in `openNeo4jModal` / `normalizedNeo4jDraft` / `saveNeo4jDraft` / `removeNeo4jConfig` / `normalizeSettings`, `testPayload('neo4j', ...)` now attaches `countryCode`/`countryCodes`
- **DB migrations:** N/A â€” reuses the existing `(tenant_id, country_code, environment, key)` unique row in `config.integration_settings` from `V12__country_environment_scoped_integration_settings.sql`. A row previously persisted as `(tenant, EG, PROD, neo4j)` keeps working; operators can move it to `ALL` simply by re-saving with All Countries selected (`save()` deletes the prior out-of-scope row deterministically).
...
- **Tests added/updated:** N/A in this change â€” existing `JdbcSettingsMetadataStoreTest` and `SettingsServiceTest` continue to cover load/save semantics, and `scopesFromItem` is already exercised by the Azure/databases/SharePoint/infrastructure list-backed paths. See follow-ups.
- **Docs updated:** `docs/PROGRESS-003.md`, `docs/API_CONTRACTS.md`, `docs/UI_PAGES.md`, `docs/SERVICES_SUPPORT.md`
- **Security / OWASP checklist:**
  - [x] No new tenant-context surface â€” country still flows from request headers, the new field only narrows persistence scope within the same tenant
  - [x] No new RBAC bypass â€” `SETTINGS_WRITE` still required by the underlying `POST .../test` and `PUT /api/v1/settings`
  - [x] No secrets exposed â€” `password`/`passwordSecret` keys remain masked/encrypted; the new country fields are non-sensitive labels
  - [x] No new SSRF surface â€” only the existing Neo4j tester URL is contacted, with the same scheme allowlist (`bolt`, `neo4j`, `bolt+s`, `neo4j+s`, `bolt+ssc`, `neo4j+ssc`)
  - [x] Cross-tenant isolation preserved â€” `JdbcSettingsMetadataStore.load` and `save` still filter by `tenant_id` first
  - [x] Cross-country isolation respected â€” a row saved at `(tenant, KW, PROD)` is not visible to a `BH` session unless an `ALL` row exists
  - [x] Audit unchanged â€” `SETTINGS_UPDATED` and `SETTINGS_TEST_REQUESTED` continue to emit secret-safe entries
- **Definition of Done checklist (from copilot-instructions Â§25):**
...
- **Author:** copilot-agent
- **Correlation:** user request â€” "also add country level for the neo4j as well"

### 2026-06-29 â€” Settings Databases popups: replace single Update/Add with Test Only + Test & Update/Save

- **Phase:** Phase 1
- **Module(s):** `commandcenter.settings` (frontend SPA)
- **Type:** feature, UX
- **Country/Tenant scope:** ALL, KW, BH, EG
- **Summary:** Aligned the Settings â†’ Databases popups with the Azure OpenAI integration popup so operators always verify connectivity before persisting. The Edit Neo4j Topology Graph popup now exposes `Cancel | Test Only | Test & Update` instead of a single `Update Neo4j` button. The Add/Edit Database, Add/Edit SharePoint, and Add/Edit Server (infrastructure) popups now expose `Cancel | Test Only | Test & Save` (or `Test & Update` when editing) instead of `Test Connection + Add/Update Connector`. The primary button runs the existing backend test endpoint (`POST /api/v1/settings/neo4j/test`, `POST /api/v1/settings/databases.connections.*/test`, `POST /api/v1/settings/sharepoint.connections.*/test`, `POST /api/v1/settings/infrastructure.connections.*/test`) and only commits the row to the section list â€” followed by the normal debounced `PUT /api/v1/settings` autosave â€” if the test returns `Pass`. A `Fail` keeps the popup open with the secret-safe failure message rendered in the test status banner so the operator can correct the input and retry without losing the form state.
- **Files touched:**
  - `src/main/resources/static/pages/settings/settings.js`
- **DB migrations:** N/A
- **API changes:** N/A â€” reuses the documented `POST /api/v1/settings/{section}/test` and `PUT /api/v1/settings` endpoints with the same payload shape; no new backend surface.
- **Tests added/updated:** N/A â€” pure UI orchestration of existing tested backend endpoints.
...
- **Security / OWASP checklist:**
  ...
  - [x] Country/environment scope continues to flow from the existing session headers â€” no new tenant-context surface
  - [x] No new SSRF surface â€” Test buttons go through the same SSRF-guarded backend test endpoints
  - [x] No new RBAC bypass â€” `SETTINGS_WRITE` is still required by the underlying `POST .../test` and `PUT /api/v1/settings`
  - [x] No raw payload echo â€” the popup only renders `status`, `latencyMs`, `message`, `checkedEndpoint` from the existing secret-safe test response
- **Definition of Done checklist (from copilot-instructions Â§25):**
  ...
  - [x] Audit logs for write actions (unchanged â€” backend still audits `SETTINGS_TEST_REQUESTED` and `SETTINGS_UPDATED`)
  - [x] No secrets exposed
  - [x] Tests for core logic (frontend orchestration of already-tested backend endpoints)
  - [x] Correlation ID supported (unchanged â€” propagated by `APIClient`)
  - [x] Does not break incident lifecycle rules
...
- **API changes:** N/A â€” restores documented `GET /api/v1/settings` behavior; no contract change. Removes the `settings metadata load unavailable` WARN that operators were seeing on every Settings page load.
- **Tests added/updated:** N/A in this change â€” existing `JdbcSettingsMetadataStoreTest` (PostgreSQL Testcontainers) covers the load/save contract. See follow-ups for a regression test that asserts a stored Settings row is read back through the SPA flow.
...
- **Security / OWASP checklist:**
  ...
  - [x] RBAC checked at service layer (unchanged â€” `SettingsService.snapshot/read/update` continue to enforce `SETTINGS_READ` / `SETTINGS_WRITE`)
  - [x] No SQL injection â€” every column reference is hard-coded; only `tenant_id` is bound as a parameter
  - [x] No secrets / PII / tokens logged or returned (secret fields still encrypted via `SecretCipherService` and masked on read)
  - [x] Cross-tenant isolation preserved â€” the `WHERE tenant_id = ?` filter is unconditional
...
- **Author:** copilot-agent
- **Correlation:** runtime observation â€” `SettingsService.loadMetadataSettings` WARN "settings metadata load unavailable â€¦ errorType=DataIntegrityViolationException" on `GET /api/v1/settings` while `config.integration_settings` already contained the persisted Azure OpenAI integration row

### 2026-06-29 â€” Wire JdbcSettingsMetadataStore so Settings persist to PostgreSQL
- **Phase:** Phase 1
- **Module(s):** `platform.config`
- **Type:** fix, security
- **Country/Tenant scope:** ALL, KW, BH, EG
...
- **Tests added/updated:** N/A â€” existing `JdbcSettingsMetadataStoreTest` (PostgreSQL Testcontainers) continues to cover load/save behavior
...
- **Security / OWASP checklist:**
  ...
  - [x] RBAC checked at service layer (unchanged â€” already in `SettingsService`)
  - [x] Inputs validated (Bean Validation) â€” no input surface changed
  - [x] Audit log written for write actions (`SETTINGS_UPDATED` already emitted by `SettingsService.update`)
  - [x] No secrets / PII / tokens logged or returned (secret fields still encrypted via `SecretCipherService` and masked on read)
  - [x] SSRF-safe for any URL-based config (unchanged â€” Settings test paths still use `SsrfGuard`)
- **Definition of Done checklist (from copilot-instructions Â§25):**
  - [x] Supports tenant + country context
  - [x] Clear DTOs and validation
  - [x] Follows package/module boundaries (`platform.config`)
...
- **Author:** copilot-agent
- **Correlation:** runtime observation â€” `SettingsService.persistMetadataSettings` warn "no SettingsMetadataStore bean available" + HTTP 503 on `PUT /api/v1/settings`

### 2026-06-29 â€” Fix Settings SPA invalid tenant-context fallback
- **Phase:** Phase 1
...
### 2026-06-29 â€” Replace H2 settings-store harness with PostgreSQL integration test
- **Phase:** Phase 1
...
### 2026-06-29 â€” Run full Maven validation and confirm remaining settings-store blocker
- **Phase:** Phase 1
- **Module(s):** `platform.config`, `build/test`
- **Type:** test, docs
- **Country/Tenant scope:** ALL, KW, BH, EG
...
|-------------------|------------------------------------------------------------------------|
| AI Providers      | `renderAzureOpenAI()` + Azure modal                                    |
...
- Modified
  ...
  - `src/main/resources/static/shared/js/layout.js` - removed the legacy `connectors` entry from `NAV_GROUPS`.
- Replaced with stubs
  - `src/main/resources/static/pages/connections/connections.js` - 1.1 KB redirect stub; UI is now rendered by the Settings module.
  - `src/main/resources/static/pages/connections/connections.css` - placeholder (styles moved into `settings.css`).
- Deleted
  - `src/main/resources/static/pages/connectors/` - entire folder (`connectors.js` 127 KB, `connectors.html`, `connectors.css` 87 KB).

### Migrations
- None (UI only).

### API changes
- None. Existing `APIClient.connectors.*` and `APIClient.settings.*` shapes preserved. Save payload matches the historical `saveConnector()` schema (`{ name, enabled, attributes: { pluginType, countryCode, environment, environmentScope, ownerTeam, authMode, baseUrl, ...pluginSpecific, secretsPlain, sharedWithTeams } }`).

### Tests / docs updated
- `node --check` clean for `settings.js`, `connections.js`, `router.js`, `config.js`.
- `.\mvnw.cmd -q -DskipTests -o package` succeeded (no Java changes).
- `docs/API_CONTRACTS.md` - no endpoint changes; UI-only redesign, no update required beyond this note.
- `docs/RUNBOOKS.md` - no operational URL changes (routes `#settings` and `#connections` still valid; `#connectors` now redirects to `#settings` Connections section via router alias).

### Security checklist (OWASP + repo section 16)
- [x] No secrets, tokens, passwords, access keys, or API keys logged or echoed in UI. All credential inputs are `type="password"`; on save/update the API returns only `secretsMask` which is what the UI displays.
- [x] `secretsPlain` is only assembled in memory for the create/update payload and never `console.log`ed or persisted client-side.
- [x] Tenant + country + user context preserved via existing `APIClient` (unchanged `X-Tenant-Id`, `X-User-Id`, `X-Correlation-Id` headers).
- [x] No PII or raw payloads written to logs.
- [x] Dev-server-only defaults in `application.properties` untouched.
- [x] No new outbound URLs; connector base-URL entry still passes through the backend `SsrfGuard` on the Java side.
- [x] Audit trail unchanged - all writes go through `APIClient.connectors.create/update/delete/toggle` which the backend already audits.

### Definition of Done checklist (repo section 25)
- [x] Supports tenant and country context (unchanged headers).
- [x] Uses DTO-shaped payloads and client-side validation (`validateConnectionDraft`).
- [x] Follows module boundaries (single UI module owns the popup; backend untouched).
- [x] Writes go through audited APIs.
- [x] Does not expose secrets.
- [x] JS parses cleanly (`node --check`).
- [x] Correlation ID propagation preserved via `APIClient`.
- [x] Does not touch incident lifecycle rules.
- [x] Does not bypass the custom index engine.
- [x] Modular structure - nothing new introduced that would block future microservice extraction.

### Follow-ups
- Backend permission API is not yet wired: the read-only info banner is stubbed and will only appear if a future `session().permissions` field explicitly excludes `CONNECTOR_WRITE`.
- `_old_connections/` remains on the working tree because two Administrator-owned files inside it cannot be removed by the current user (`Access is denied` on `del /F`). Git already sees the original tracked files as deleted; the orphan dir is unreferenced by the router. Recommend an admin one-off cleanup at deployment time.
- The "Manage External Requests" button in the popup's warning banner is a benign no-op that navigates to the System Variables section - replace with a real allowlist screen when backend policy exists.
- "Share access" tab currently only writes `attributes.sharedWithTeams`; backend enforcement of team-scoped connector visibility is still TODO.

### Author
Copilot CLI (`ui-settings` / `ui-connections` refactor)