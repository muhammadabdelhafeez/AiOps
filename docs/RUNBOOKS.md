...
- Configure the connector with the SCOM management server FQDN/IP or WinRM URL ending in `/wsman`, domain, WinRM port (`5986` for HTTPS or `5985` for HTTP), **Negotiation type** (Kerberos or NTLM), lookback hours, timeout, and write-only username/password.
- **Negotiation type (Kerberos vs NTLM):** use **NTLM** when the collector host can't reach a domain controller — Kerberos then fails with `0x80090311` ("domain isn't available"). NTLM authenticates with the explicit username/password via WinRM **Negotiate** (SPNEGO) without a KDC. The connector persists `authMethod` = `Kerberos` or `NTLM`; the collector maps `NTLM → -Authentication Negotiate` (PowerShell has no literal `NTLM` value). Both the Test probe and the ingestion bridge honor it.
...
- **Unified slim page header (`.kfh-phdr`)** — every page renders one consistent ~54px header (title + subtitle + search + optional scope/filters/sort/actions) defined once in `shared/css/kfh-design-system.css` (`.kfh-phdr*`, `.kfh-fchip`). Any page containing a `.kfh-phdr` auto-hides the shell `#global-bar` via `main.kfh-main-content:has(.kfh-phdr) #global-bar{display:none}`, so there is exactly one header per page. Applied to Dashboard, Incidents, Alerts, Log Explorer, Applications, Inventory, Reports, Users, Audit, Schedules and Settings. NOTE: the shared stylesheet was renamed from `kfh-dynatrace-system.css` → `kfh-design-system.css` (no third-party product names in shipped code); the roadmap doc is `docs/UI_PARITY_ROADMAP.md`.
  - Incidents (`pages/incidents/incidents.js`) → `GET /api/v1/incidents` → **produced live by `CorrelationService.correlateWindow`** over `kfh.incidents.window-hours` (default 24h): it reads the indexed alerts, groups them by shared causal path over the topology, and `IncidentService.list` maps each `CorrelatedIncident` (root cause + impacted apps + evidence) to the detail shape. Read-model fallback if correlation is unavailable.
    - **Topology auto-map** (`kfh.topology.auto-map`, default true): a CI not in the seeded/CMDB topology is auto-registered by `TopologyService` as its own single-node component (stable id `auto:<CI>`), so real BMC/SCOM alerts still form incidents (one per affected CI, no dependency grouping / no impacted apps) instead of being dropped as an `[CORRELATE] unmapped CI(s)` CMDB gap. Set `TOPOLOGY_AUTO_MAP=false` to require real CMDB coverage. Expanding the seeded topology / wiring a CMDB restores dependency-based grouping + impacted-application blast radius.
