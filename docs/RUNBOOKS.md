...
- Passwords submitted by the Create User, Edit User optional password fields, or the User table **Password** reset action are hashed with BCrypt on the backend. Do not log request bodies for `/api/v1/users`, `/api/v1/users/{id}/password`, or `/api/v1/auth/sign-in`.
...
- `GLOBAL_ADMIN` receives `*` permissions and can use All groups / country switching. To create a user that starts directly in all-country scope, open User Management as a global admin, choose **All countries** in the Create User country selector, and assign **Admin**. The user then signs in with `countryCode = "ALL"`; physical country users continue to sign in with `KW`, `BH`, or `EG`.
...
- The page calls `GET /api/v1/audit` and displays only real application actions recorded by authentication and write flows such as successful/failed login, user management, app settings update/test, connector, schedule, alert acknowledgement, incident, inventory, application, report, and bootstrap identity provisioning operations. It does not render seeded, sample, generated, or dummy audit rows.
- If the page is empty, perform a real action (for example sign in, intentionally fail a sign-in test with a non-production account, update settings, test a connector, run a schedule, update a user, or generate a report), then click **Refresh**. Empty state is expected on a freshly started Phase 1 in-memory read model before any auditable action occurs.
